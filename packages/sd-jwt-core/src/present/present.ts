...
export const present = async <T extends Record<string, unknown>>(
  ...
  const presentedDisclosures = keys.map((k) => hashmap[disclosureKeymap[k]]).filter((d) => d !== undefined)

  return [jwt, ...presentedDisclosures.map((d) => d.encode()), kbJwt ?? ''].join(SD_SEPARATOR)
}
 ...
 * @returns
 */
export const transformPresentationFrame = (obj: PresentationFrame<Extensible>, prefix: string[] = []): string[] => {
  return Object.entries(obj).reduce<string[]>((acc, [key, value]) => {
    ...
    if (typeof value === 'boolean') {
      ...
      acc.push(encodedPrefix, ...transformPresentationFrame(value as PresentationFrame<Extensible>, newPrefix))
    }
    return acc
  }, [])
}
...
export const createHashMappingForSerializedDisclosure = (disclosures: SerializedDisclosure[]) => {
  ...
  for (let i = 0; i < disclosures.length; i++) {
    ...
    map[digest] = Disclosure.fromArray(key ? [salt, key, value] : [salt, value], { digest, encoded })
  }
  return map
}
