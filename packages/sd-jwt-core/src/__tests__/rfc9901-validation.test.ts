...
const makeDisclosure = (digest: string, key: string | undefined, value: unknown) =>
...
describe('RFC 9901 validation', () => {
  ...
  test('rejects duplicate digest in _sd array', () => {
    ...
    expect(() => unpackObj(payload, map)).toThrow('Duplicate digest found in SD-JWT payload')
  })
  ...
  test('rejects unreferenced disclosure', () => {
    ...
    expect(() => unpackObj(payload, map)).toThrow('Unreferenced disclosure(s) detected in SD-JWT')
  })
  ...
  test('rejects disclosed claim name that conflicts with plaintext key', () => {
    ...
    expect(() => unpackObj(payload, map)).toThrow('Disclosed claim name "name" conflicts with existing payload key')
  })
