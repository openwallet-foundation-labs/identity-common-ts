...
import { unpackObj } from '../decode'
import { GeneralJSON, Jwt, KBJwt, SDJwtInstance } from '../index'
import { selectDisclosures, transformPresentationFrame } from '../present'
...
describe('RFC 9901 audit fixes', () => {
  test('rejects malformed disclosure containers', () => {
    ...
    expect(() => unpackObj({ _sd: 'object-digest' }, {})).toThrow('Invalid _sd claim: expected array of strings')
    expect(() => unpackObj({ items: [{ '...': 'array-digest', extra: true }] }, {})).toThrow(
      'Invalid array disclosure placeholder'
    )
    expect(() => unpackObj({ _sd: ['array-digest'] }, { 'array-digest': arrayDisclosure })).toThrow(
      'Array disclosure cannot be used as an object property'
    )
    expect(() => unpackObj({ items: [{ '...': 'object-digest' }] }, { 'object-digest': objectDisclosure })).toThrow(
  ...
  test('rejects disclosed claim name collisions at the same object level', () => {
    ...
    expect(() => unpackObj({ _sd: ['digest-1', 'digest-2'] }, { 'digest-1': first, 'digest-2': second })).toThrow(
      'Disclosed claim name "name" conflicts with another disclosure'
    )
  })
  ...
  test('validates JWT claims after disclosure processing', async () => {
    ...
    const payload = Buffer.from(JSON.stringify({ _sd: [disclosureDigest], _sd_alg: 'sha-256' })).toString('base64url')
  ...
  test('rejects unsafe JWT algorithm and invalid validity claims', async () => {
    ...
    await expect(jwt.verify(() => true)).rejects.toThrow('Verify Error: alg "none" is not allowed')
    ...
    await expect(signed.verify(verifier)).rejects.toThrow('Verify Error: JWT exp must be a number')
  })
  ...
  test('validates expected issuer audience', async () => {
    ...
    await expect(jwt.verify(verifier, { expectedAudience: 'verifier-b' })).rejects.toThrow(
      'Verify Error: Invalid audience'
    )
  })
  ...
  test('rejects duplicate salts during issuance', async () => {
    ...
    await expect(sdjwt.issue({ first: 'a', second: 'b' }, { _sd: ['first', 'second'] })).rejects.toThrow(
      'Duplicate disclosure salt detected'
    )
  })
  ...
  test('rejects disclosures and kb_jwt outside the first General JSON header', () => {
    ...
    ).toThrow('disclosures and kb_jwt MUST only appear in the first unprotected header')
  })

  test('escapes presentation paths with literal dots in claim names', () => {
    expect(transformPresentationFrame({ 'a.b': true })).toEqual(['a~1b'])
    expect(transformPresentationFrame({ a: { b: true } })).toEqual(['a', 'a.b'])
