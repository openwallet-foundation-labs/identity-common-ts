...
describe('Disclosure', () => {
  ...
  test('digest disclosure #2', async () => {
    const disclosure = new Disclosure(['2GLC42sKQveCfGfryNRN9w', 'given_name', 'John'])
    const encode = disclosure.encode()
    expect(encode).toBe('WyIyR0xDNDJzS1F2ZUNmR2ZyeU5STjl3IiwiZ2l2ZW5fbmFtZSIsIkpvaG4iXQ')
  ...
  test('digest disclosure #3', async () => {
    ...
    const disclosure = await Disclosure.fromEncode(TestDataDraft7.claimTests[0].disclosure, hash)
  ...
  test('digest disclosure #4', async () => {
    for (const sha256Test of TestDataDraft7.sha256Tests) {
      const disclosure = await Disclosure.fromEncode(sha256Test.disclosure, hash)
