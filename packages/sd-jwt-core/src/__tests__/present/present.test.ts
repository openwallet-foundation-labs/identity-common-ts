...
describe('Present tests', () => {
  ...
  test('presentableKeys sync', () => {
    ...
    const keys = presentableKeysSync(decodedSdJwt.jwt.payload, decodedSdJwt.disclosures, digest)
    expect(keys).toStrictEqual(['arr', 'arr.0', 'arr.2.a', 'foo', 'test.zzz'])
  })
  ...
  test('expect missing digest', () => {
    ...
    expect(() => selectDisclosures(payload, disclosures, presentationFrame)).toThrowError(
