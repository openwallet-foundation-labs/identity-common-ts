...
describe('decode tests', () => {
  ...
  test('split rejects bare jwt without SD-JWT separator', () => {
    const sdjwt = 'h.p.s'
    expect(() => splitSdJwt(sdjwt)).toThrow('Invalid SD-JWT: missing SD-JWT separator')
  })
  ...
  test('decode rejects bare jwt without SD-JWT separator', async () => {
    ...
    await expect(decodeSdJwt(jwt, digest)).rejects.toThrow('Invalid SD-JWT: missing SD-JWT separator')
  })
  ...
  test('decode sync rejects bare jwt without SD-JWT separator', () => {
    const jwt =
      'eyJhbGciOiJIUzI1NiIsInR5cCI6InNkK2p3dCJ9..mX14Sw86xy8NFQta7tCfNmhVCqzfaJ_K3VEIhTjbLDY'
    expect(() => decodeSdJwtSync(jwt, digest)).toThrow('Invalid SD-JWT: missing SD-JWT separator')
  })
  ...
  test('get claims sync', () => {
    const sdjwt =
      'eyJ0eXAiOiJzZC1qd3QiLCJhbGciOiJFZERTQSJ9.eyJfc2QiOlsiaWQ1azZ1ZVplVTY4bExaMlU2YjJJbF9QR3ZKb1RDMlpkMkpwY0RwMzFIWSJdLCJfc2RfYWxnIjoic2hhLTI1NiJ9.GiLF_HhacrstqCJ223VvWOoJJWU8qk4dYQHklSMwxv36pPF_7ER53Wbty1qYRlQ6NeMUdBRRdj9JQLLCzz1gCQ~WyI2NTMxNDA2ZmVhZmU0YjBmIiwiZm9vIiwiYmFyIl0~'
    const decodedSdJwt = decodeSdJwtSync(sdjwt, digest)
    const claims = getClaimsSync(decodedSdJwt.jwt.payload, decodedSdJwt.disclosures, digest)
    expect(claims).toStrictEqual({
      foo: 'bar',
    })
  })
