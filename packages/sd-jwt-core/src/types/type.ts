...
export const encodePathSegment = (segment: string) => segment.replace(/~/g, '~0').replace(/\./g, '~1')

export const encodePath = (segments: string[]) => segments.map(encodePathSegment).join('.')
...
export type KbVerifier = (data: string, sig: string, payload: JwtPayload) => OrPromise<boolean>
