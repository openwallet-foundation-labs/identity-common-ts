...
export class Disclosure<T = unknown> {
  ...
  public static async fromEncode<T>(s: string, hash: HasherAndAlg) {
    ...
    const item = decodeBase64urlJsonStrict<DisclosureData<T>>(s, 'Invalid disclosure data')
  ...
  public static fromArray<T>(item: DisclosureData<T>, _meta?: { digest: string; encoded: string }) {
