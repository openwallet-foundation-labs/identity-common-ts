...
import { DeviceResponse, SessionTranscript } from '../../..'
...
describe('Animo mdoc 0.5.x mdoc implementation', () => {
  test('verify DeviceResponse from Paradym Wallet (OpenID4VP 1.0) issued by Animo Playground using mdoc 0.5.x', async () => {
    ...
    expect(
      new X509Certificate(decoded.documents?.[0].issuerSigned.issuerAuth.certificate as Uint8Array<ArrayBuffer>).equal(
        issuerCertificate
      )
    ).toBe(true)
