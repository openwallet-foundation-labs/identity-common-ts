...
import { CoseKey, HpkeSuiteId, type MdocContext } from '..'
...
export const mdocContext: MdocContext = {
  ...
  x509: {
    getSubjectNameField: (input) => {
      const certificate = new X509Certificate(input.certificate as Uint8Array<ArrayBuffer>)
    ...
    verifyCertificateChain: async (input) => {
      ...
      const parsedLeafCertificate = new x509.X509Certificate(certificateChain[0] as Uint8Array<ArrayBuffer>)
      const certificatesToBuildChain = [...certificateChain, ...(trustedCertificates ?? [])].map(
        (c) => new x509.X509Certificate(c as Uint8Array<ArrayBuffer>)
      )
      ...
      if (trustedCertificates) {
        const parsedTrustedCertificates = trustedCertificates.map(
          (trustedCertificate) => new X509Certificate(trustedCertificate as Uint8Array<ArrayBuffer>)
        )
    ...
    getCertificateData: async (input: { certificate: Uint8Array }) => {
      const certificate = new X509Certificate(input.certificate as Uint8Array<ArrayBuffer>)
      const thumbprint = await certificate.getThumbprint(crypto as unknown as Crypto)
