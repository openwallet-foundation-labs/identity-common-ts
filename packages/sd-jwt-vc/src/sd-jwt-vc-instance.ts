import { isMediaType, secondsToDate } from '@owf/identity-common'
import {
...
export class SDJwtVcInstance extends SDJwtInstance<SdJwtVcPayload> {
  ...
  private async statusListFetcher(uri: string): Promise<string> {
    ...
    try {
      const response = await fetch(uri, {
        ...
        headers: { Accept: MediaTypes.StatusListJwt },
      })
      if (!response.ok) {
      ...
      if (!isMediaType(response.headers.get('content-type'), MediaTypes.StatusListJwt)) {
   ...
   * @returns
   */
  async getVct(encodedSDJwt: string): Promise<ResolvedTypeMetadata | undefined> {
    // Call the parent class's verify method
    const { payload, header } = await SDJwt.extractJwt<Record<string, unknown>, SdJwtVcPayload>(encodedSDJwt)
   ...
   * @returns
   */
  private async fetchWithIntegrity(url: string, integrity?: string): Promise<unknown> {
    try {
   ...
   * @returns
   */
  private async fetchVct(result: VerificationResult): Promise<ResolvedTypeMetadata | undefined> {
    const typeMetadataFormat = await this.fetchSingleVct(result.payload.vct, result.payload['vct#integrity'])
   ...
   */
  private mergeTypeMetadata(base: TypeMetadataFormat, extending: TypeMetadataFormat): TypeMetadataFormat {
  ...
  private async resolveVctExtendsChain(
    ...
    if (extendedTypeMetadata.extends) {
      resolvedTypeMetadata = await this.resolveVctExtendsChain(extendedTypeMetadata, depth + 1, visitedVcts)
    } else {
    ...
    const mergedTypeMetadata = this.mergeTypeMetadata(resolvedTypeMetadata.mergedTypeMetadata, parentTypeMetadata)

    return {
      mergedTypeMetadata: mergedTypeMetadata,
      typeMetadataChain: [parentTypeMetadata, ...resolvedTypeMetadata.typeMetadataChain],
   ...
   * @returns
   */
  private async fetchSingleVct(vct: string, integrity?: string): Promise<TypeMetadataFormat | undefined> {
    const fetcher = this.userConfig.vctFetcher ?? ((uri, integrity) => this.fetchWithIntegrity(uri, integrity))
