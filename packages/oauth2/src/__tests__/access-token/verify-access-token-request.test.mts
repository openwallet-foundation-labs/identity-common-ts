...
describe('Verify Pre Authorized Code Access Token Request', () => {
...
describe('Verify Authorization Code Access Token Request', () => {
  ...
  test('handles valid authorization code request', async () => {
    const dpopPrivateJwk = {
      kty: 'EC',
      d: 'UxBOEoXuH9qlZ0Bo2E1sCuZDkAzVl99eenarvWMrgH0',
      crv: 'P-256',
...
describe('Verify Refresh Token Access Token Request', () => {
  test('handles refresh token not matching', async () => {
    await expect(
      verifyRefreshTokenAccessTokenRequest({
        authorizationServerMetadata,
        accessTokenRequest: {
    ...
    ).rejects.toThrow(`Invalid 'refresh_token' provided`)
  })

  test('handles refresh token expired', async () => {
    const now = new Date()
    await expect(
      verifyRefreshTokenAccessTokenRequest({
        authorizationServerMetadata,
        ...
        request,
        refreshTokenExpiresAt: new Date(now.getTime() - 100),
        now,
      })
    ).rejects.toThrow(`Expired 'refresh_token' provided`)
  })
  ...
  test('handles valid authorization code request', async () => {
    ...
    const { dpop } = await verifyRefreshTokenAccessTokenRequest({
      authorizationServerMetadata,
      accessTokenRequest: {
        grant_type: refreshTokenGrantIdentifier,
        code: 'hello2',
      ...
      // 1 minute
      refreshTokenExpiresAt: new Date(now.getTime() + 60000),
      now,
      dpop: {
        required: true,
        jwt: dpopJwt,
