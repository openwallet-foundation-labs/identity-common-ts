...
import { decodeSdJwt, getClaims, present, presentableKeys } from '@sd-jwt/core'
...
;(async () => {
  ...
  const keys = await presentableKeys(decodedSdJwt.jwt.payload, decodedSdJwt.disclosures, digest)
  ...
  const presentedClaims = await getClaims(presentedDecodedSdJwt.jwt.payload, presentedDecodedSdJwt.disclosures, digest)
